...
# Build notes (appended — read this part too)

The sections above are the original brief. This section records the state of the
implementation against it, so a future session does not have to re-derive it.

## Where each phase stands

- **Phase 0 (shell)** — done. Camera flow with a pre-permission explainer, device
  picker, mirror toggle, landmark overlay in a worker, settings persistence,
  light/dark/high-contrast, unplug/replug recovery.
- **Phase 1 (fingerspelling)** — done, with a geometric baseline rather than a
  trained MLP (no dataset was available). Dwell-time commit, auto-space,
  confusion-aware autocomplete, backspace, J/Z motion head, and an in-browser
  calibration flow that fits both prototypes and a linear head.
- **Phase 2 (word signs)** — partially done. The temporal feature pipeline,
  segmenter, few-shot recorder and rejection band all work; the 150-sign target
  vocabulary is proposed but **no trained model ships**, and the UI says so.
- **Phase 3 (reverse)** — done, clip-based. The gloss engine is a real
  translation with visible rules. The clip manifest ships empty, so everything
  fingerspells. The avatar path is deliberately not implemented.
- **Phase 4 (conversation)** — scaffolded and honest. CTC decoder and
  gloss→English rules exist; no model behind them; off by default.
- **Phase 5 (polish)** — done. PWA, offline model caching, transcript export,
  keyboard shortcuts, docs, model-card infrastructure.

## Constraints that are enforced in code, not just documented

- `privacy.onDeviceOnly` is typed as literal `true` and re-forced on every
  settings write and every migration.
- `models/registry.ts` refuses any ONNX file whose SHA-256 does not match its
  manifest entry.
- An E2E test asserts the word "interpreter" only ever appears in a disclaiming
  context.
- The disclaimer component has no dismiss affordance, by design.

## If you are about to change something

- `src/features/normalize.ts` is the contract every model is trained against.
...
- Anything holding per-hand state across frames keys on `HandFrame.id` from
  `vision/tracking.ts`, never on handedness or list position. MediaPipe flips
  labels and reorders the list, and keying on either throws the state away.
- `features/scanQuality.ts` may only ever *withhold* output. It has no path to
  raising a confidence or forcing a commit, and that is the point of it.
...
- Read `docs/ETHICS.md` before adding anything to Reverse or Conversation mode.

## Section 12 answers

Not asked — the build was requested as one uninterrupted run. Defaults were
assumed and are recorded, with their consequences, at the top of
`docs/DECISIONS.md`. All six are cheap to revisit.
